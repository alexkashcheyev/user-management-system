...
|---|---|---|---|---|
| `GET` | `/users` | `limit`, `offset` | A list of User objects, max length is `limit`, first `offset` entities are skipped | `404` if no users exist after given `offset`, `200` otherwise |
| `POST` | `/users` | `firstName`, `lastName`, `email`, `password`, `description` | Saves a new user with given parameters in the database and returns a URL to the created user. | `201` with `Location` header |
| `GET` | `/users/{id}` | | The user identified by given `id` | `200` if a user with given `id` exists, `404` if it doesn't |
| `PUT` | `/users/{id}` | `firstName`, `lastName`, `email`, `password`, `description` | Updates data for the user with given `id` and returns the updated user | `200` if the user exists, `404` if it doesn't |
